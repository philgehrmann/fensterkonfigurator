      ...
      <ul className="grid grid-flow-col justify-self-center" key="navigation">
        {content.navigation.map((item, index) => {
          return !item.subnav ? (
            ...
            <div>
              <li
                className="grid content-center grid-flow-col px-4 h-[73px] text-[16px] border-b-2 border-transparent hover:border-b-2 hover:border-black font-[500]  group hover:bg-bermuda hover:text-white cursor-pointer"
                data-hover={item.subnavIdent}
                key={index}
